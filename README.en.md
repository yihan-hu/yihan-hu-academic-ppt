# Guizang PPT Skill · HTML / Editable PPTX / Images / Covers
...
Yes. Hybrid editable PPTX is supported: titles, body text, numbers, cards, hairlines, simple charts, and normal images stay as native PowerPoint objects, while browser-only WebGL/canvas/complex CSS/maps are rasterized only as local visual layers. Use `both` to export HTML + PPTX together.
