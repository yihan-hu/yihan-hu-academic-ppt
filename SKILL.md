...
>
> **PPTX design-source P0:** set `meta.designSource: "guizang-template"`, `meta.pptxFidelity: "native-first"`, and `meta.figurePolicy: "whitelist-enforced"`. Use Guizang parent geometry/typography/spacing. Keep ordinary titles, body text, tables, panels, hairlines and normal slide layout native. **Before drawing a scientific visual, classify it against `references/figure-generation-whitelist.md`; a whitelist hit must be a coherent picture object by default. Do not bypass this by omitting `figureKind`.** Forest/effect plots are deterministic/preserved. Methods/study-design/conceptual/state/cohort illustrations are generated/rendered as figure images by default. If the user explicitly asks to edit the internals of a non-quantitative explanatory figure and the Canva connector is available, read `references/canva-editable-figures.md` and use Canva Magic Layers (`image-to-design`) as an optional editable companion workflow. **Canva editability is not equivalent to native PowerPoint editability:** do not mark a figure PowerPoint-editable unless the exported PPTX is separately verified as native/selectable. Quantitative/statistical figures, exact axes, intervals, risk tables, and evidence geometry must never be reconstructed through Magic Layers. Rasterize ordinary slide text only for genuine browser-only effects, never as a layout shortcut.
>
> **KI preset P0:** the skill ships two locked KI derivatives: `assets/template-ki-swiss.html` (`brandProfile: "ki-swiss"`) and `assets/template-ki-editorial.html` (`brandProfile: "ki-editorial"`). Read `references/ki-templates.md` before any KI deck. These files remain Guizang-locked: KI Swiss differs only by approved theme tokens; KI Editorial differs only by approved theme tokens plus the single whitelisted light-page `reading-ink` color-role binding. Logos remain brand assets, not layout sources. If the user says only "KI" and the family is not already clear from context, ask `KI x Swiss` or `KI x Electronic Magazine`.

> **Brand asset P0:** logos and other official brand marks must preserve their intrinsic aspect ratio. In deck-spec use `role: "logo"` and `fit: "contain"`; never stretch a logo to fill an arbitrary box. Normal content slides must keep the logo out unless explicitly allowed by the selected brand cadence.

> **KI editorial table P0:** for `ki-editorial` native tables, read `references/ki-editorial-tables.md`. Use the Defense-derived filled table grammar: very pale pink-grey header, first body row white, next body row near-white pink-grey, then alternate. Keep borders secondary and mainly horizontal; do not recreate a dense spreadsheet grid. Keep HR/OR/RR and 95% CI in one cell; use same-size bold for important estimates and do not create a duplicate giant statistic unless explicitly requested.

> **KI editorial color P0:** Guizang owns layout/typography/spacing; KI/Defense owns the color roles. Read `references/ki-editorial-visual-grammar.md`. Use `#FFFFFF` light pages; on light pages, ordinary slide titles default to KI magenta `#840050`, while body text remains `#111111` and secondary text uses `#6F6B6D`. Use `#840050` for compact semantic labels, numbering, monoline icons/arrows and key navigation cues, never ordinary paragraphs. Use `#4F0433` for semantic dark pages and, on a light slide, at most one bounded `semantic-band` with white text. Use `#F7F3F5` for subtle equal peer panels, `#EFE8EB` for stronger grouping/header/macro fields, `#D9D9D9` only as hairline/separator, and `#FD8169` only for semantic result/risk highlights. Override Guizang Style-A automatic dark/light cycling: dark slides are semantic (cover/section/synthesis/closing), not periodic content pages.

> **KI editorial macro-field P0:** do not fix light-page emptiness by removing all fill or by adding many small pale cards. Read `references/ki-editorial-macro-fields.md`. Use Guizang-style visual mass: oversized type anchors, continuous figure/table grounds, or **selective** macro-fields. Three genuinely equal peers may use three restrained `#F7F3F5` peer panels without a macro-field; `#EFE8EB` macro-fields remain optional and a minority rhythm. If a macro-field is used for three comparable peer groups, default to a **middle** macro-field; use side 2+1 macro-fields only for an explicit source/story semantic asymmetry. Run pale-fill area QA to prevent overcorrection into a pink-grey deck.

> **KI final palette QA P0:** after PPTX normalization, run `scripts/check-ki-pptx-palette.py` for the active KI preset. Reject old or over-heavy KI Editorial colors: `#870052 / #F1F3F5 / #E4E8EC / #E6DDE2 / #F3E8EE / #FAF5F8 / #F5EEF2 / #DDD0D5 / #8F587B`, and reject excessive deep-plum dark pages.

> **PPTX font P0:** use installed editable fallbacks for KI Editorial PPTX: `Noto Serif Display` (display), `Noto Sans` (body), `Noto Sans Mono` (mono/meta). Do not request unavailable Playfair Display / IBM Plex Mono in PPTX; do not permit visible text to fall back to Georgia / Courier New.

> **Audience-readable text P0:** most text that the audience is expected to read during the talk should land around **14–16 pt at final PowerPoint size**. This applies especially to body copy, table cells, forest/Love plot row labels, axis labels, estimate columns, and diagram labels. Titles remain larger. Secondary annotations may use 12–14 pt; going below 12 pt is an exception that requires a real density constraint, deliberate visual review, and preferably shortening/splitting before further font reduction. Read `references/text-readability.md` and, for generated figures, `references/figure-render-qa.md`.

> **Scientific figure render-QA P0:** generated scientific figures with embedded text must pass `references/figure-render-qa.md`. Deterministic SVG/HTML/vector figures are not assumed safe merely because the source looks correct: SVG `<text>` does not auto-wrap, font metrics can change across renderers, and Office may render embedded SVG unreliably. Manually wrap bounded labels, rasterize the figure alone for inspection, inspect every figure slide individually after Office/PPTX rendering, and re-check the normalized final deck. For target environments with SVG compatibility risk, embed the validated deterministic figure as high-resolution PNG. Run `scripts/check-svg-text-bounds.py figure.svg` before rasterizing deterministic SVG figures with bounded labels, and run `scripts/check-pptx-figure-media.py <deck.pptx> --fail-on-svg` when SVG blanking has been observed or cross-Office compatibility matters.

---

# Magazine Web + Hybrid Pptx
...
生成一份演讲型 deck，支持 **单文件 HTML**、**混合可编辑 PPTX**，或 **HTML + PPTX 双输出**，并提供两种可选的视觉基调：
...
**两种风格共享**：HTML 模式支持横向翻页（键盘 ← →、滚轮、触屏、ESC 索引）、Lucide 图标、Motion One 入场动效（本地 + CDN 双保险）。

### 输出模式（先定格式，再动手）

- `html`：保留现有单文件网页 deck。
- `pptx`：生成**混合可编辑 PPTX**；标题、正文、数字、卡片、线条、基础图表用 PowerPoint 原生元素，WebGL/canvas/复杂 CSS/地图等只栅格化视觉层。
- `both`：HTML 与 PPTX 共用同一内容计划；PPTX 额外维护 `deck-spec.json` 作为可编辑元素和几何的来源。

用户明确说 `PowerPoint` / `.pptx` / `可编辑 PPT` 时必须走 `pptx` 或 `both`，不要用整页截图冒充 PPTX。具体契约先读 `references/pptx-hybrid.md`。
...
- 大段表格数据、图表叠加（用常规 PPT）
- 培训课件（信息密度不够）
- HTML-only 模式不适合需要多人协作编辑；这类场景优先输出混合可编辑 PPTX

## 工作流

### Step 1 · 需求澄清(**动手前必做**)

**如果用户已经给了完整的大纲 + 图片/截图处理要求 + 输出格式**,可以跳过澄清；PPTX/both 仍从 Step 1.5 开始，HTML-only 直接进 Step 2。
...
| 2 | **输出格式?**(`html` / `pptx` / `both`) | 决定是否建立 `deck-spec.json`、是否需要局部 raster layer、以及最终交付物 |
...
### Step 1.5 · 建立双渲染内容模型（PPTX / both 必做）

如果输出包含 PPTX，先读 `references/pptx-hybrid.md`，在目标目录创建 `deck-spec.json`：

- slide 顺序、可编辑文字、坐标、图片槽位、原生 shape/line 都写进 spec。
- WebGL、canvas、复杂 CSS/SVG、MapLibre 等浏览器专属视觉不写成整页截图；在 HTML 中只给这些复杂层加 `data-pptx-raster`，全局背景 canvas 用 `data-pptx-raster-global`。
- PPTX 坐标统一使用 16:9 wide 画布（13.333333 × 7.5 inch）。
- 同一页默认渲染顺序：背景色 → raster underlay → 原生 editable elements → 必要时 raster overlay。

PPTX 中必须保持可编辑的内容：主标题、正文、数字、caption、卡片、hairline、简单图表/时间线、普通图片。

### Step 2 · 拷贝模板

**如果输出是 `html` 或 `both`，根据 Step 1 选定的风格拷贝对应模板**到目标位置（通常是 `项目/XXX/ppt/index.html`），同时在同级建一个 `images/` 文件夹准备接图片。

如果输出是 `pptx`：
- 页面需要 WebGL/canvas/复杂 CSS 等浏览器视觉时，仍拷贝 HTML 模板作为 raster staging source；
- 页面全部能用 PowerPoint 原生元素表达时，可以不保留最终 HTML，但仍必须维护 `deck-spec.json`。
...
### Step 3.3 · PPTX 混合渲染（输出包含 PPTX 时必做）

先读 `references/pptx-hybrid.md`。原则是**能原生就原生，只栅格化浏览器专属视觉**。

1. 根据当前 layout 把标题、正文、KPI、卡片、hairline、时间线、简单图表和普通图片写入 `deck-spec.json` 的 `elements[]`。
2. 如果存在 WebGL/canvas/ASCII/复杂 CSS/地图，在 HTML 中给这些视觉节点加 `data-pptx-raster`；模板全局背景 canvas 已带 `data-pptx-raster-global`。
3. 准备只保留浏览器特效层的 capture HTML：

```bash
python <SKILL_ROOT>/scripts/prepare-pptx-raster-html.py path/to/index.html path/to/raster-source
```

4. 用当前环境的浏览器截图工具以 16:9（建议 1600×900）渲染 `raster-source/slide-XX.html`，保存为 `raster/slide-XX-raster.png`；然后把对应 PNG 填入 slide 的 `rasterUnderlay` 并生成 PPTX：

```bash
node <SKILL_ROOT>/scripts/render-pptx.mjs path/to/deck-spec.json path/to/deck.pptx
```

5. 验证 PPTX 包结构、slide 数量、原生文字节点、素材路径和越界：
...
**硬规则**：
- 不要把一整页 HTML 截图铺满 PPTX 后再声称“可编辑”。
- raster underlay 存在时，除纯视觉封面等明确例外，页面仍应有原生 editable elements。
- 重要文字不得藏进 `data-pptx-raster`。
- 图片先按模板要求裁成标准比例；PPTX renderer 的 `fit=stretch` 只用于已经匹配槽位比例的图片，截图保真可用 `fit=contain`。
- Motion/WebGL 动态在 PPTX 默认取静态最终态；只有用户明确要求才额外设计 PowerPoint 动画。

在 OpenAI 的 slide 环境中，PPTX 生成与 QA 还必须遵循已安装 `slides` skill：使用 PptxGenJS，并运行其可用的 render/overflow 检查工具。

### Step 4 · 对照检查清单自检

生成完一定要打开 `references/checklist.md`，逐项对照；如果包含 PPTX，同时执行 `references/pptx-hybrid.md` 的 hybrid quality gates。里面总结了**真实迭代过程中踩过的所有坑**，P0 级别的问题（emoji、图片撑破、标题换行、字体分工）必须全部通过。
...
### Step 5 · 本地预览 / PPTX 视觉核对

HTML 输出直接在浏览器打开 `index.html`。macOS 下：
...
如果输出包含 PPTX，还必须把 `deck.pptx` 渲染成逐页图片或 PDF 做视觉核对，确认：字体替换、文字溢出、图片拉伸、raster/native 重复、遮挡、越界都不存在。不能只验证文件“能打开”。
...
├── scripts/
│   ├── validate-swiss-deck.mjs ← 风格 B HTML 静态校验
│   ├── prepare-pptx-raster-html.py ← 生成“只保留浏览器特效层”的逐页截图源 HTML
│   ├── render-pptx.mjs          ← deck-spec.json → 混合可编辑 PPTX（PptxGenJS）
│   └── check-pptx-hybrid.py     ← PPTX 包结构、原生文字、素材和越界检查
└── references/
    ...
    ├── pptx-hybrid.md        ← HTML/PPTX 双渲染契约、native/raster 决策和 deck-spec 格式
    └── checklist.md          ← 质量检查清单（P0/P1/P2/P3 分级）
```

**加载顺序建议**：
1. 先读完 `SKILL.md`(这个文件)了解整体。
2. Step 1 先确定风格 A/B 和输出模式 `html` / `pptx` / `both`；再读对应的 `themes*.md`。
3. 如果输出包含 `pptx`，立即读 `pptx-hybrid.md`，先建立 `deck-spec.json` 和 native/raster 分工。
4. 如果需要 HTML（最终交付或仅作为 raster staging），动手前 Read 对应模板的 `<style>` 块——这是 HTML 类名的唯一来源。
5. 读对应 layouts 文件挑布局：风格 A → `layouts.md`；风格 B → 先读 `swiss-layout-lock.md`，再读 `layouts-swiss.md`。
6. 风格 B 需要地点、路线、人物住所或城市关系地图时，读 `swiss-map-component.md`。
7. 生成配图时读 `image-prompts.md`；用户原始截图先读 `screenshot-framing.md`。
8. 细节调整时读 `components.md`（风格 B 组件细节主要在 `layouts-swiss.md` 附录）。
9. 生成后：HTML Swiss 跑 `validate-swiss-deck.mjs`；PPTX 跑 `check-pptx-hybrid.py`；最后统一读 `checklist.md` 做视觉自检。
...
- `html`：交付 `index.html` + `images/` 所需素材。
- `pptx`：交付 `deck.pptx`；建议同时保留 `deck-spec.json` 和生成脚本/素材，便于继续编辑与再生成。
- `both`：交付 `index.html` + `deck.pptx` + `deck-spec.json`，两者页序和核心内容必须一致。
- 如果 PPTX 依赖 raster layer，保留 `raster/` 源 PNG；这些 PNG 只承载浏览器专属视觉，不应承载本该可编辑的正文。
...
## Academic fork entrypoint reminder

在执行以上 Guizang 全量规则的同时，Academic 任务必须读取并执行 `references/academic-overlay.md`；机构品牌任务额外执行 `references/brand-overlay.md` 与 `references/brand-profile.md`。任何未被这些 overlay 明确覆盖的 Guizang 规则不得修改、弱化或重新解释。

Academic brand/PPTX mandatory tools:
- `scripts/inspect-brand-template.py`: inspect real PPTX colors/media before choosing a brand token.
- `scripts/apply-brand-profile.py`: mechanically replace Guizang accent variables while preserving Guizang color-role constraints.
- `scripts/check-brand-token-lock.py`: reject palette drift, residual old accent and unauthorized second accents.
- `scripts/render-academic-pptx.mjs`: render the native-first PowerPoint slide; logos/evidence/generated figures are aspect-safe images.
- `scripts/check-academic-pptx-hybrid.py`: validate Guizang layout identity, brand lock, figure whitelist and foreground collisions.
- `scripts/check-pptx-layout-integrity.py`: inspect the actual normalized PPTX for severe text overlap, image distortion, logo collisions and out-of-bounds shapes.
- `scripts/finalize-pptx.py`: normalize through a real office engine before delivery.

`scripts/capture-guizang-visual-plates.py` is legacy/browser-effect support only. Do not use it for ordinary Academic slide layout or text.

Bundled brand profiles live in `references/brands/`; `ki` is available for Karolinska Institutet and uses the supplied KI reference only for branding, not layout.
