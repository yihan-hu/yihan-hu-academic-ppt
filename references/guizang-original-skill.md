...
description: 生成演讲型 PPT，支持单文件 HTML 横向翻页 deck、混合可编辑 PPTX，或同时输出两者。PPTX 模式用原生 PowerPoint 文本/形状/图片承载标题、正文、数据、卡片与基础图表，并把 WebGL、canvas、复杂 CSS/地图等浏览器特效局部栅格化为透明视觉层。提供两种风格：① "电子杂志 × 电子墨水" ② "瑞士国际主义"。当用户提到网页 PPT、PowerPoint/PPTX、可编辑 PPT、杂志风 PPT、瑞士风 PPT、Swiss Style、horizontal swipe deck，或要求 HTML + PPTX 双输出时使用。
---
