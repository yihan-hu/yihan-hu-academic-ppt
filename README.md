# Guizang PPT Skill · HTML / 可编辑 PPTX / 配图 / 封面
...
![Hybrid PPTX](https://img.shields.io/badge/PPTX-Hybrid%20Editable-143CFF?style=flat-square)
...
一个适配 Claude Code / Codex 等 Agent 环境的演讲 deck 技能，用于生成**单文件 HTML 横向翻页 PPT**、**混合可编辑 PPTX**、PPT 配图和多平台封面。PPTX 中标题/正文/数字/卡片/线条/普通图片保持原生可编辑，WebGL、canvas、复杂 CSS 与地图只局部栅格化。
...
帮我把这份内容做成瑞士风可编辑 PPTX，WebGL/复杂视觉可以局部栅格化。
同一份内容输出 HTML + PPTX 两个版本。
...
- 🧩 **混合可编辑 PPTX**:文本、数字、卡片、线条、基础图表与普通图片保留为 PowerPoint 原生元素；WebGL/canvas/复杂 CSS/地图只做透明 raster layer
- 🔁 **双输出模式**:`html` / `pptx` / `both`，PPTX 由 `deck-spec.json` 驱动，避免整页截图式转换

## 适合 / 不适合

**✅ 合适**:线下分享 / 行业内部讲话 / 私享会 / AI 产品发布 / demo day / 带强烈个人风格的演讲

**❌ 不合适**:大段表格数据 / 传统培训课件(信息密度不够)。需要多人协作时不要选 HTML-only，优先用混合可编辑 PPTX。
...
- **需要可编辑时不再绕路**:PPTX 模式直接生成 PowerPoint 原生文本/形状/图片，不把整页 HTML 烤成截图。
- **复杂视觉仍能保留**:WebGL、canvas、复杂 CSS、地图等浏览器专属层可以单独栅格化后作为 underlay/overlay 进入 PPTX。
...
**可以导出 PPTX 吗?**
可以。现在支持混合可编辑 PPTX：标题、正文、数字、卡片、hairline、简单图表和普通图片尽量使用 PowerPoint 原生元素；WebGL/canvas/复杂 CSS/MapLibre 等浏览器专属视觉只局部栅格化。也可以选择 `both` 同时输出 HTML + PPTX。
